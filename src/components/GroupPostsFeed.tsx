...
type GroupPostsFeedProps = {
  posts: GroupFeedPost[];
  onPostsChange: Dispatch<SetStateAction<GroupFeedPost[]>>;
  emptyMessage?: string;
  collapseCount?: number;
  onOpenGroup?: (post: GroupFeedPost) => void;
};
...
const isPubliclyShareablePost = (post: Pick<GroupFeedPost, "visibility">) => {
...
export default function GroupPostsFeed({
  ...
  const buildShareUrl = useCallback((post: GroupFeedPost, postKey: string) => {
  ...
  const trackShare = useCallback(
    async (post: GroupFeedPost, postKey: string) => {
      try {
  ...
  const handleCopyShare = useCallback(
    async (post: GroupFeedPost, postKey: string, shareUrl: string) => {
      if (!shareUrl) {
        pushShareNotice(postKey, "Unable to copy link.");
        return;
      }
      if (!isPubliclyShareablePost(post)) {
        pushShareNotice(postKey, "This group is private. Only public posts can be shared externally.");
        return;
      }
      try {
  ...
  const handleNativeShare = useCallback(
    async (post: GroupFeedPost, postKey: string, shareUrl: string, shareText: string) => {
      if (!navigator.share) {
  ...
  const handleReaction = useCallback(
    async (post: GroupFeedPost, postKey: string, emoji: string) => {
  ...
  const renderCommentsModal = (post: GroupFeedPost) => {
  ...
  const renderPostCard = (post: GroupFeedPost, openable = true) => {
    ...
    const shareText = post.title
      ? `${post.ownerName || "Member"}: ${post.title}`
      : `${post.ownerName || "Member"} posted in ${post.groupName || "a group"}.`;
    ...
    return (
      ...
      >
        <div className="post-meta-bar">
          <span className="post-meta-name">{post.ownerName || "Member"}</span>
          ...
          {post.groupName &&
            (onOpenGroup ? (
              ...
              <span className="post-meta-tag">{post.groupName}</span>
            ))}
        </div>
        {post.imageUrl ? (
